...
class ScrapeAllrecipesService
  ...
  def call
    url = URI.open("https://www.allrecipes.com/search?q=#{@ingredient}")
    doc = Nokogiri::HTML.parse(File.open(url), nil, "utf-8")
    results = []
    ...
    doc.search(".mntl-card-list-items.card").each do |element|
      ...
      description = details_doc.search("p").first.text.strip
